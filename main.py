...
from crud import get_user_by_username, create_user, increment_login_attempts, reset_login_attempts
...
def login_user(user: UserLogin, db: Session = Depends(get_db)):
    ...
    if not pwd_context.verify(user.password, db_user.hashed_password):
        increment_login_attempts(db, db_user)
        remaining_attempts = 3 - db_user.login_attempts
    ...
    reset_login_attempts(db, db_user)
